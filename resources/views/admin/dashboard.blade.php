<x-default-layout title="Dashboard" section_title="Dashboard">
  <!-- Main Content -->
  <main class="space-y-6 p-6">
    <!-- Statistics Cards -->
    <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
      <!-- Total Events -->
      <div class="overflow-hidden rounded-lg bg-white shadow-md transition-transform hover:translate-y-[-2px]">
        <div class="flex items-start justify-between p-6">
          <div>
            <p class="text-sm font-medium text-gray-500">Total Events</p>
            <p class="mt-1 text-2xl font-bold text-gray-800">{{ $events->count() }}</p>
          </div>
          <div class="rounded-lg bg-blue-500 p-3">
            <i data-lucide="calendar" class="text-white"></i>
          </div>
        </div>
      </div>

      <!-- Total Buyers -->
      <div class="overflow-hidden rounded-lg bg-white shadow-md transition-transform hover:translate-y-[-2px]">
        <div class="flex items-start justify-between p-6">
          <div>
            <p class="text-sm font-medium text-gray-500">Total Buyers</p>
            <p class="mt-1 text-2xl font-bold text-gray-800">{{ $buyers->count() }}</p>
          </div>
          <div class="rounded-lg bg-purple-500 p-3">
            <i data-lucide="users" class="text-white"></i>
          </div>
        </div>
      </div>

      <!-- Total Revenue -->
      <div class="overflow-hidden rounded-lg bg-white shadow-md transition-transform hover:translate-y-[-2px]">
        <div class="flex items-start justify-between p-6">
          <div>
            <p class="text-sm font-medium text-gray-500">Total Revenue</p>
            <p class="mt-1 text-2xl font-bold text-gray-800">
              Rp {{ number_format($purchases->sum(fn ($p) => $p->qty * $p->event->price), 0, ',', '.') }}
            </p>
          </div>
          <div class="rounded-lg bg-green-500 p-3">
            <i data-lucide="dollar-sign" class="text-white"></i>
          </div>
        </div>
      </div>
    </div>

    <!-- Events Table -->
    <div class="rounded-lg bg-white p-6 shadow-md">
      <h2 class="mb-4 text-xl font-semibold text-gray-800">Events</h2>
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">#</th>
              <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Title</th>
              <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Date</th>
              <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Location</th>
              <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">
                Tickets Sold
              </th>
              <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Revenue</th>
              <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Actions</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200 bg-white">
            @foreach ($events as $index => $event)
              <tr class="hover:bg-gray-50">
                <td class="px-6 py-4 text-sm whitespace-nowrap text-gray-500">{{ $index + 1 }}</td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm font-medium text-gray-900">{{ $event->title }}</div>
                </td>
                <td class="px-6 py-4 text-sm whitespace-nowrap text-gray-500">
                  {{ \Carbon\Carbon::parse($event->event_date)->format('d M Y H:i') }}
                </td>
                <td class="px-6 py-4 text-sm whitespace-nowrap text-gray-500">
                  {{ $event->location }}
                </td>
                <td class="px-6 py-4 text-sm whitespace-nowrap text-gray-500">
                  {{ $event->purchases->sum('qty') }}
                </td>
                <td class="px-6 py-4 text-sm whitespace-nowrap text-gray-500">
                  Rp
                  {{ number_format($event->purchases->sum(fn ($p) => $p->status == 'paid' ? $p->qty * $event->price : 0), 0, ',', '.') }}
                </td>
                <td class="px-6 py-4 text-sm font-medium whitespace-nowrap">
                  <div class="flex space-x-2">
                    <a href="{{ route('admin.events.show', $event->id) }}" class="text-blue-600 hover:text-blue-900">
                      <i data-lucide="eye"></i>
                    </a>
                    <a
                      href="{{ route('admin.events.edit', $event->id) }}"
                      class="text-yellow-600 hover:text-yellow-900"
                    >
                      <i data-lucide="edit"></i>
                    </a>
                    <form
                      action="{{ route('admin.events.destroy', $event->id) }}"
                      method="POST"
                      onsubmit="return confirm('Are you sure you want to delete this event?')"
                    >
                      @csrf
                      @method('DELETE')
                      <button type="submit" class="text-red-600 hover:text-red-900">
                        <i data-lucide="trash"></i>
                      </button>
                    </form>
                  </div>
                </td>
              </tr>
            @endforeach
          </tbody>
        </table>
      </div>
    </div>

    <!-- Two Column Layout for Buyers and Purchases -->
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
      <!-- Recent Buyers -->
      <div class="rounded-lg bg-white p-6 shadow-md">
        <h2 class="mb-4 text-xl font-semibold text-gray-800">Recent Buyers</h2>
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Name</th>
                <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Email</th>
                <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Phone</th>
                <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Joined</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 bg-white">
              @foreach ($buyers->take(10) as $buyer)
                <tr class="hover:bg-gray-50">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm font-medium text-gray-900">{{ $buyer->name }}</div>
                  </td>
                  <td class="px-6 py-4 text-sm whitespace-nowrap text-gray-500">
                    {{ $buyer->email }}
                  </td>
                  <td class="px-6 py-4 text-sm whitespace-nowrap text-gray-500">
                    {{ $buyer->phone }}
                  </td>
                  <td class="px-6 py-4 text-sm whitespace-nowrap text-gray-500">
                    {{ $buyer->created_at->diffForHumans() }}
                  </td>
                </tr>
              @endforeach
            </tbody>
          </table>
        </div>
      </div>

      <!-- Recent Purchases -->
      <div class="rounded-lg bg-white p-6 shadow-md">
        <h2 class="mb-4 text-xl font-semibold text-gray-800">Recent Purchases</h2>
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Buyer</th>
                <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Event</th>
                <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Qty</th>
                <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">Status</th>
                <th class="px-6 py-3 text-left text-xs font-medium tracking-wider text-gray-500 uppercase">
                  Purchased At
                </th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 bg-white">
              @foreach ($purchases as $purchase)
                <tr class="hover:bg-gray-50">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm font-medium text-gray-900">
                      {{ $purchase->buyer->name }}
                    </div>
                  </td>
                  <td class="px-6 py-4 text-sm whitespace-nowrap text-gray-500">
                    {{ $purchase->event->title }}
                  </td>
                  <td class="px-6 py-4 text-sm whitespace-nowrap text-gray-500">
                    {{ $purchase->qty }}
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <span
                      class="{{ $purchase->status === 'paid' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }} inline-flex rounded-full px-2 text-xs leading-5 font-semibold"
                    >
                      {{ ucfirst($purchase->status) }}
                    </span>
                  </td>
                  <td class="px-6 py-4 text-sm whitespace-nowrap text-gray-500">
                    {{ \Carbon\Carbon::parse($purchase->purchased_at)->format('d M Y H:i') }}
                  </td>
                </tr>
              @endforeach
            </tbody>
          </table>
        </div>
      </div>
    </div>
  </main>
</x-default-layout>
